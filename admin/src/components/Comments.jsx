...
useEffect(()=> {
  ...
  return (
    <>
        <Modal opened = {openComment} onClose = {handleClose}
                    ...
                    <div className='w-full h-full flex flex-col gap-6 px-2 '>
                        {data?.data?.map(({ _id, user, desc, post, createdAt}) => {
                            <div key={_id} className='w-full flex gap-4'>
                                <img src={user?.image || NoProfile} alt="Profile" className='w-10 h-10 rounded-full object-cover ' />

                                <div className='w-full flex justify-between'>
                                    <div className='w-full flex items-center gap-2'>
                                        <p className='text-slate-600 dark:text-gray-400 font-medium'>{user.name}</p>
